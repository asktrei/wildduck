...
const { da } = require('./translations');
const axios = require('axios');

const accountSid = 'AC9527b38bbdd22c18322520a5e0bd1a60';
const authToken = '6e5fa6121cf83c01e33bace8d46f440a';
...
class UserHandler {
    ...
    async setupOtpsms(user, data) {
        ...
        let enabled2fa = Array.isArray(userData.enabled2fa) ? userData.enabled2fa : [].concat(userData.enabled2fa ? 'otpsms' : []);

        if (enabled2fa.includes('otpsms')) {
            let err = new Error('OTP SMS 2FA is already enabled for this user');
            err.code = 'OtpsmsEnabled';
            throw err;
        }

        // let sms = 123456;
        let smsExpired = new Date();
        
        // const postData = {
        //     'username': 'gmi-mail',
        //     'secret': 'DY?bxHkzUec&WXWE4&TJ4gukGWZf?2JnHzrv#pV7kG@+vXdfVg*4TVL7%Av4n8qb$GEmpa_wLD#dg?x?TvmWCJcWzfTM38NcAK==N-NKaJF-J849N@XZ-+NU#NtxP&K$S-rHRy*8YfMVnPBt@PqTK%LVt!56R^S3YdfxN!W%zB&?6@z5!MbuQa@VWP8C722A-e?hm!Qdgxa+Nn*-Qjm3PMvdRjnWd@H?XQ_auV^B@mN89Ay3^JU5gX?qpnWq@#3M',
        //     'phone': userData.phoneCountryID+userData.phone
        // };

        // const response = await axios.post('http://25.30.15.7:2020/requestSend', postData)

        client.verify.services('VA4da356babf20f758a61e94ca19b3fe3e')
             .verifications
             .create({to: userData.phoneCountryID+userData.phone, channel: 'sms'})
        ...
        try {
            result = await this.users.collection('users').updateOne(
                {
                    _id: user,
                    enabled2fa: { $not: { $eq: 'otpsms' } }
                },
                {
                    $set: {
                        sms : 100000,
                        smsExpired : smsExpired
                    }
                },
                { maxTimeMS: consts.DB_MAX_TIME_USERS }
            );
    ...
    async enableOtpsms(user, data) {
        ...
        verificationResult = client.verify.services('VA4da356babf20f758a61e94ca19b3fe3e')
             .verificationChecks
             .create({to: userData.phoneCountryID+userData.phone, code: data.sms});

        if ((await verificationResult).status != "approved") {
            try {
                await this.logAuthEvent(user, {
                    action: 'enable 2fa otpsms',
                    result: 'fail',
                    sess: data.sess,
                    ip: data.ip
                });
            ...
            err.code = 'OTP SMS 2FA is not initialized for this user';
            throw err;
        }

        // if (!userData.sms || (userData.sms && userData.smsExpired < new Date(Date.now() - TOTP_SETUP_TTL))) {
        //     // 2fa not set up
        //     let err = new Error('OTP SMS 2FA is not initialized for this user');
        //     err.code = 'OtpsmsDisabled';
        //     throw err;
        // }

        // if (userData.sms != data.sms) {
        //     try {
        //         await this.logAuthEvent(user, {
        //             action: 'enable 2fa otpsms',
        //             result: 'fail',
    ...
    async checkOtpsms(user, data) {
        ...
        if (!enabled2fa.includes('otpsms')) {
            // 2fa not set up
            let err = new Error('OTP SMS 2FA is not already enabled for this user');
            err.code = 'OtpsmsNotEnabled';
            throw err;
        }
        ...
        verificationResult = client.verify.services('VA4da356babf20f758a61e94ca19b3fe3e')
             .verificationChecks
             .create({to: userData.phoneCountryID+userData.phone, code: data.token});

        if ((await verificationResult).status != "approved") {
            try {
                await this.logAuthEvent(user, {
                    action: 'check 2fa otpsms',
                    result: 'fail',
                    sess: data.sess,
                    ip: data.ip
                });
    ...
    async sendOtpsms(user, data) {
        ...
        if (!enabled2fa.includes('otpsms')) {
            // 2fa not set up
            let err = new Error('OTP SMS 2FA is not enabled for this user');
            err.code = 'OtpsmsNotEnabled';
            throw err;
        }
        ...
        if (!result || !result.matchedCount) {
            let err = new Error('Could not update user, check if 2FA is not already sent');
            err.code = 'OtpsmsNotSent';
            throw err;
        }

        try {
            await this.logAuthEvent(user, {
                action: 'send 2fa otpsms',
                result: 'success',
                sess: data.sess,
                ip: data.ip
            });
    ...
    async disableOtpsms(user, data) {
        ...
        if (!enabled2fa.includes('otpsms')) {
            let err = new Error('Could not update user, check if 2FA OTP SMS is not already disabled');
            err.code = 'OtpsmsDisabled';
            throw err;
        }
