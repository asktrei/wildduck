...
echo "#!/bin/bash
git --git-dir=/var/opt/zonemta-wildduck.git --work-tree=/opt/zone-mta/plugins/gmi-mail checkout "\$3" -f
cd /opt/zone-mta/plugins/gmi-mail
...
mkdir -p /opt/zone-mta/plugins/gmi-mail
git --git-dir=/var/opt/zonemta-wildduck.git --work-tree=/opt/zone-mta/plugins/gmi-mail checkout "$WILDDUCK_ZONEMTA_COMMIT"
...
echo '# @include "/etc/gmi-mail/dbs.toml"' > /etc/zone-mta/dbs-production.toml
echo 'user="gmi-mail"
group="gmi-mail"' | cat - /etc/zone-mta/zonemta.toml > temp && mv temp /etc/zone-mta/zonemta.toml
...
" > /etc/zone-mta/plugins/gmi-mail.toml
...
chown -R gmi-mail:gmi-mail /etc/zone-mta

# Ensure required files and permissions
echo "d /opt/zone-mta 0755 deploy deploy
d /etc/zone-mta 0755 gmi-mail gmi-mail" > /etc/tmpfiles.d/zone-mta.conf
log_script "zone-mta"
