...
echo 'cert="/etc/gmi-mail/certs/fullchain.pem"
key="/etc/gmi-mail/certs/privkey.pem"' > /etc/gmi-mail/tls.toml

sed -i -e "s/key=/#key=/g;s/cert=/#cert=/g" /etc/zone-mta/interfaces/feeder.toml
echo '# @include "../../gmi-mail/tls.toml"' >> /etc/zone-mta/interfaces/feeder.toml
...
/root/.acme.sh/acme.sh --issue --nginx \
    -d "$HOSTNAME" \
    --key-file       /etc/gmi-mail/certs/privkey.pem  \
    --fullchain-file /etc/gmi-mail/certs/fullchain.pem \
